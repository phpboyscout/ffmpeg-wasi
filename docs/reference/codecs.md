...
`audio_codec`), by [profile](variants.md). Every codec here is in-tree libavcodec, with no external
...
All are **software, single-threaded**, and VP9 encode in particular is slow without threads, so
...
- **HEVC/AV1 encode** live in the **full** profile below (native only). **AV1 *decode*** is via
  **libdav1d** (spec 0023 D-0023-C), in the **intermediate** profile (and full), **both variants,
  ...
  runs on wazero, which has no threads, so decode is correct but slower than the threaded native
...
The **heavy next-generation encoders** (spec 0023). They are thread- and SIMD-hungry and impractical
