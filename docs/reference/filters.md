...
`avfilter_graph_parse2`. A filter can be used only if it is **enabled in the build**, and this is
...
all in-tree, LGPL-clean, and needing no external library:
...
- **`palettegen`/`paletteuse`** run as a single-pass split graph, so pair them for a good GIF:
...
- **`eq`** (a GPL-only level/gamma filter) is **not enabled in either variant**, so reach for
