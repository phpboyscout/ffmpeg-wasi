...
!!! note "Two `process` validation failures exited `0` up to n8.1.2-12"
    ...
    their error to stderr and then exited **`0`** with empty stdout, rather than the malformed-request code
...
| `process: encoder <name> does not have option <key>` | `1` | a key in `outputs[].options` the encoder did not consume. **All three option dictionaries are strict** , because an option that is not applied would otherwise produce output built to different settings with exit `0`, which the caller cannot detect |
...
| `frames: bad filter chain "<s>"` | `1` | the `scale` value is spliced into a filtergraph, so check its syntax (`"320:-2"`) |
