...
The engine ships as a single binary, either a `wasm32-wasi` module or a native ELF `driver` (both built
...
- **A `Read` reply of `0` means end-of-file**, not a short read, and the driver turns it into
...
Write mode must open `O_RDWR` (not append), so a muxer's backward seeks (for example the non-fragmented
