...
| `meta.c` | ~90 | **Metadata and chapters**: shared helpers for reading/writing container and per-stream tags, disposition flags, and chapters. Used by both `probe` (read) and `process` (write). |
