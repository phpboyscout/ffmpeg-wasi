...
The usual escape (switching to a thread-capable runtime such as Wasmtime) means **CGO**, a C
...
validated, and exactly as capable as the engine actually is, with no leaky partial-CLI illusion.

It also means we **own a current FFmpeg build**, which is genuine porting work (wasm has no threads,
